...
public abstract class TypeInfo implements HasName, HasEnclosingType, HasMethods, HasAnnotations, HasTypeParameters {
    ...
    public static TypeInfo of(TypeMirror typeMirror) {
        ...
        TypeInfo type = of(typeMirror.toString());
        if (typeMirror instanceof DeclaredType) {
            DeclaredType declaredType = (DeclaredType) typeMirror;
            if (declaredType.getEnclosingType().getKind() != TypeKind.NONE) {
                type = type.toBuilder().enclosingType(declaredType.getEnclosingType()).build();
            } else {
                Element element = declaredType.asElement().getEnclosingElement();
                if (element instanceof TypeElement) {
                    type = type.toBuilder().enclosingType(MoreElements.asType(element)).build();
                }
            }
        }
